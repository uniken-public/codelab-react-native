...
import type {
  ...
  RDNAIDVAdditionalDocumentScanCallback,
  RDNAHandleCustomChallengeData,
  RDNAHandleCustomChallengeCallback
...
class RdnaEventManager {
  ...
  // IDV handlers
  private idvAdditionalDocumentScanHandler?: RDNAIDVAdditionalDocumentScanCallback;
  
  // Custom challenge handlers
  private handleCustomChallengeHandler?: RDNAHandleCustomChallengeCallback;
  ...
  private registerEventListeners() {
    ...
    console.log('🔧 RdnaEventManager - Native modules available:', !!NativeModules.RdnaClient);
    ...
    this.listeners.push(
      ...
      // IDV event listeners
      this.rdnaEmitter.addListener('onIDVAdditionalDocumentScan', this.onIDVAdditionalDocumentScan.bind(this)),
      
      // Custom challenge event listeners
      
    );
    
    console.log('🔧 RdnaEventManager - Native event listeners registered');
    console.log('🔧 RdnaEventManager - Total listeners registered:', this.listeners.length);
    console.log('🔧 RdnaEventManager - onIDVAdditionalDocumentScan listener registered successfully');
  }
  ...
  /**
   * Handles IDV additional document scan response events
   * @param response Raw response from native SDK containing additional document scan data
   */
  private onIDVAdditionalDocumentScan(response: RDNAJsonResponse) {
    console.log("🔥 RdnaEventManager - IDV additional document scan response event received");
    console.log("🔥 RdnaEventManager - Raw response:", JSON.stringify(response, null, 2));

    try {
      const additionalDocScanData: RDNAIDVAdditionalDocumentScanData = JSON.parse(response.response);
      console.log("🔥 RdnaEventManager - Parsed additional document scan data:", JSON.stringify(additionalDocScanData, null, 2));
      console.log("🔥 RdnaEventManager - IDV additional document scan status:", additionalDocScanData.challengeResponse?.status?.statusCode);
      console.log("🔥 RdnaEventManager - Challenge mode:", additionalDocScanData.challengeMode);
      console.log("🔥 RdnaEventManager - Document version:", additionalDocScanData.idvResponse?.version);
      console.log("🔥 RdnaEventManager - Document type:", additionalDocScanData.idvResponse?.document_type);

      console.log("🔥 RdnaEventManager - Handler exists:", !!this.idvAdditionalDocumentScanHandler);
      
      if (this.idvAdditionalDocumentScanHandler) {
        console.log("🔥 RdnaEventManager - Dispatching to main thread and calling handler");
        this.dispatchToMainThread(() => {
          console.log("🔥 RdnaEventManager - About to call idvAdditionalDocumentScanHandler");
          this.idvAdditionalDocumentScanHandler?.(additionalDocScanData);
          console.log("🔥 RdnaEventManager - Called idvAdditionalDocumentScanHandler successfully");
        });
      } else {
        console.error("🔥 RdnaEventManager - No handler registered for IDV additional document scan!");
      }
    } catch (error) {
      console.error("🔥 RdnaEventManager - Failed to parse IDV additional document scan response:", error);
      console.error("🔥 RdnaEventManager - Raw response that caused error:", response.response);
    }
  }
  ...
  // IDV Handler Setters
  public setIDVAdditionalDocumentScanHandler(callback?: RDNAIDVAdditionalDocumentScanCallback): void {
    console.log('🔧 RdnaEventManager - Setting IDV additional document scan handler:', !!callback);
    console.log('🔧 RdnaEventManager - Previous handler existed:', !!this.idvAdditionalDocumentScanHandler);
    this.idvAdditionalDocumentScanHandler = callback;
    console.log('🔧 RdnaEventManager - Handler set successfully, handler now exists:', !!this.idvAdditionalDocumentScanHandler);
  }

  // Custom Challenge Handler Setters

  public setHandleCustomChallengeHandler(callback?: RDNAHandleCustomChallengeCallback): void {
    this.handleCustomChallengeHandler = callback;
  }
  ...
  public cleanup() {
    ...
    // Clear IDV handlers
    this.idvAdditionalDocumentScanHandler = undefined;
    
    // Clear custom challenge handlers
    this.handleCustomChallengeHandler = undefined;
