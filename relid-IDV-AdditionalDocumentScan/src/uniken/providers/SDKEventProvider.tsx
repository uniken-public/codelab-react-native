...
export const SDKEventProvider: React.FC<SDKEventProviderProps> = ({ children }) => {
  ...
  const handleGetUserConsentForLDA = useCallback((data: RDNAGetUserConsentForLDAData) => {
    ...
    NavigationService.navigateOrUpdate('UserLDAConsentScreen', {
      eventName: 'getUserConsentForLDA',
      eventData: data,
  ...
  /**
   * Event handler for IDV document scan process start confirmation requests
   */
  const handleGetIDVDocumentScanProcessStartConfirmation = useCallback(async (data: RDNAGetIDVDocumentScanProcessStartConfirmationData) => {
    ...
    console.log('SDKEventProvider - Document type:', data.documentType);
    
    try {
      // Call getIDVConfig API before navigation
      ...
      // Following the same pattern as VerifyAuthScreen navigation
  ...
  const handleGetIDVBiometricOptInConsent = useCallback((data: RDNAGetIDVBiometricOptInConsentData) => {
    ...
    // Following the same pattern as reference SaveTemplate navigation
  ...
  /**
   * Event handler for custom challenge requests
   * Navigates to appropriate challenge screen based on challenge type
   */
  const handleCustomChallenge = useCallback((data: RDNAHandleCustomChallengeData) => {
    console.log('SDKEventProvider - Handle custom challenge event received');
    console.log('SDKEventProvider - Challenge name:', data.challengeObject?.chlng_name);
    console.log('SDKEventProvider - User ID:', data.userID);
    console.log('SDKEventProvider - Status code:', data.status.statusCode);
    console.log('SDKEventProvider - Error code:', data.error.longErrorCode);
    
    // Check if this is a NationalID challenge and navigate to dedicated screen
    if (data.challengeObject?.chlng_name === 'NationalID' && 
        data.error.longErrorCode === 0 && 
        data.status.statusCode === 100) {
      
      console.log('SDKEventProvider - Navigating to NationalID challenge screen');
      
      // Navigate to National ID Challenge screen
      NavigationService.navigateOrUpdate('NationalIDChallengeScreen', {
        challengeData: data,
        title: 'National ID Verification',
        subtitle: `Enter your National ID for verification, ${data.userID}`,
      });
      
    } else {
      console.log('SDKEventProvider - Custom challenge not handled or has errors:', {
        challengeName: data.challengeObject?.chlng_name,
        errorCode: data.error.longErrorCode,
        statusCode: data.status.statusCode
      });
      
      // For non-NationalID challenges or error conditions, show a generic message
      Alert.alert(
        'Custom Challenge', 
        `Challenge type "${data.challengeObject?.chlng_name}" not supported or has errors.`
      );
    }
  }, []);
  ...
  useEffect(() => {
    ...
    // IDV event handlers
    console.log('🔧 SDKEventProvider - Setting up IDV event handlers');
    ...
    // Custom challenge event handlers
    eventManager.setHandleCustomChallengeHandler(handleCustomChallenge);
