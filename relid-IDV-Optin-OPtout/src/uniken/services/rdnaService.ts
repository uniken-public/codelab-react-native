...
export class RdnaService {
  ...
  async getSDKVersion(): Promise<string> {
    return new Promise((resolve, reject) => {
      RdnaClient.getSDKVersion(response => {
        ...
        try {
          const version = (response as any)?.response || 'Unknown';
  ...
  async takeActionOnThreats(modifiedThreatsJson: string): Promise<RDNASyncResponse> {
    return new Promise((resolve, reject) => {
      RdnaClient.takeActionOnThreats(modifiedThreatsJson as any, response => {
  ...
  /**
   * Confirms IDV opt-in captured frame for biometric enrollment
   *
   * This method submits the user's confirmation of the captured biometric frame during IDV opt-in process.
   * It processes the user's decision to accept or reject the captured frame.
   * After successful API call, the SDK will trigger the onIDVOptInCapturedFrameConfirmation event.
   * Uses sync response pattern similar to other API methods.
   *
   * Response Validation Logic (following reference app pattern):
   * 1. Check error.longErrorCode: 0 = success, > 0 = error
   * 2. An onIDVOptInCapturedFrameConfirmation event will be triggered with confirmation results
   * 3. Async events will be handled by event listeners
   *
   * @param isConfirm User's frame confirmation decision (true = accept frame, false = retake)
   * @param challengeMode Challenge mode for the frame confirmation
   * @returns Promise<RDNASyncResponse> that resolves with sync response structure
   */
  async confirmIDVOptInCapturedFrame(isConfirm: boolean, challengeMode: number): Promise<RDNASyncResponse> {
    return new Promise((resolve, reject) => {
      console.log('RdnaService - Confirming IDV opt-in captured frame:', { isConfirm, challengeMode });
      
      // Note: This assumes the native SDK has a method to confirm captured frames.
      // Since the callback exists in native code, there should be a corresponding API method.
      // The method name may vary - check your native SDK documentation.
      if (typeof RdnaClient.confirmIDVOptInCapturedFrame === 'function') {
        RdnaClient.confirmIDVOptInCapturedFrame(isConfirm, challengeMode, response => {
          console.log('RdnaService - ConfirmIDVOptInCapturedFrame sync callback received');

          const result: RDNASyncResponse = response as any;
          
          if (result.error && result.error.longErrorCode === 0) {
            console.log('RdnaService - ConfirmIDVOptInCapturedFrame sync response success, waiting for frame confirmation event');
            resolve(result);
          } else {
            console.error('RdnaService - ConfirmIDVOptInCapturedFrame sync response error:', result);
            reject(result);
          }
        });
      } else {
        // Fallback if method doesn't exist in native SDK yet
        console.warn('RdnaService - confirmIDVOptInCapturedFrame method not available in native SDK');
        console.warn('RdnaService - Please implement this method in the native layer if needed');
        reject(new Error('confirmIDVOptInCapturedFrame method not available in native SDK'));
      }
    });
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      RdnaClient.updateNotification(
        ...
        result => {      // syncCallback
          ...
          const syncResponse: RDNASyncResponse = result as any;
   ...
   *
   * This method initiates the biometric opt-in process for users who want to enable
   * biometric template storage for their account. It allows users to consent to
   * biometric data collection and storage for future authentication.
   * After successful API call, the SDK will trigger relevant IDV events.
   * Uses sync response pattern similar to other API methods.
   *
   * Response Validation Logic (following reference app pattern):
   * 1. Check error.longErrorCode: 0 = success, > 0 = error
   * 2. Relevant IDV events will be triggered for the biometric opt-in flow
   * 3. Async events will be handled by event listeners
   *
   * @param reason The reason for initiating biometric opt-in (e.g., "User Biometric Enrollment")
   * @returns Promise<RDNASyncResponse> that resolves with sync response structure
   */
  async initiateIDVBiometricOptIn(reason: string): Promise<RDNASyncResponse> {
    return new Promise((resolve, reject) => {
      ...
      // Using initiateActivatedCustomerKYC as a base for biometric opt-in
      // In a real implementation, this would be replaced with the actual SDK method
   ...
   *
   * This method initiates the biometric opt-out process for users who want to disable
   * or remove their stored biometric templates from their account. It allows users to
   * revoke consent for biometric data storage and removes existing biometric data.
   * After successful API call, the SDK will trigger relevant IDV events.
   * Uses sync response pattern similar to other API methods.
   *
   * Response Validation Logic (following reference app pattern):
   * 1. Check error.longErrorCode: 0 = success, > 0 = error
   * 2. Relevant IDV events will be triggered for the biometric opt-out flow
   * 3. Async events will be handled by event listeners
   *
   * @param reason The reason for initiating biometric opt-out (e.g., "User Biometric Removal")
   * @returns Promise<RDNASyncResponse> that resolves with sync response structure
   */
  async initiateIDVBiometricOptOut(reason: string): Promise<RDNASyncResponse> {
    return new Promise((resolve, reject) => {
      ...
      // Using initiateActivatedCustomerKYC as a base for biometric opt-out
      // In a real implementation, this would be replaced with the actual SDK method
  ...
  /**
   * Sets IDV biometric opt-in confirmation response
   * Used as a response to the onIDVOptInCapturedFrameConfirmation callback
   * 
   * @param status Status - 0: APPROVED, 1: RECAPTURE, 2: CANCEL
   * @returns Promise<RDNASyncResponse> that resolves with sync response structure
   */
  async setIDVBiometricOptInConfirmation(status: number): Promise<RDNASyncResponse> {
    ...
    return new Promise((resolve, reject) => {
      RdnaClient.setIDVBiometricOptInConfirmation(status, (response: any) => {
        ...
        console.log('RdnaService - SetIDVBiometricOptInConfirmation sync raw response', response);
