...
import type {
  ...
  RDNAIDVBiometricOptInStatusCallback,
  RDNAIDVBiometricOptOutStatusData,
  RDNAIDVBiometricOptOutStatusCallback,
  RDNAIDVOptInCapturedFrameConfirmationData,
  RDNAIDVOptInCapturedFrameConfirmationCallback,
...
class RdnaEventManager {
  ...
  // IDV biometric status handlers
  private idvBiometricOptInStatusHandler?: RDNAIDVBiometricOptInStatusCallback;
  private idvBiometricOptOutStatusHandler?: RDNAIDVBiometricOptOutStatusCallback;
  private idvOptInCapturedFrameConfirmationHandler?: RDNAIDVOptInCapturedFrameConfirmationCallback;
   ...
   * @param response Raw response from native SDK containing biometric opt-in status details
   */
   ...
   * @param response Raw response from native SDK containing biometric opt-out status details
   */
   ...
   * @param response Raw response from native SDK containing captured frame confirmation details
   */
  private onIDVOptInCapturedFrameConfirmation(response: RDNAJsonResponse | any) {
    ...
    console.log("RdnaEventManager - Response type:", typeof response);
    console.log("RdnaEventManager - Response keys:", Object.keys(response || {}));
    ...
    try {
      ...
      if (typeof response === 'string') {
        ...
        try {
          ...
          console.error("RdnaEventManager - Failed to parse string response:", parseError);
          console.log("RdnaEventManager - Raw string response:", response.substring(0, 200) + "...");
          return;
        }
      } else if (response?.response) {
        // Standard RDNAJsonResponse format
        if (typeof response.response === 'string') {
          try {
            idvOptInCapturedFrameData = response.response;
          } catch (parseError) {
            console.error("RdnaEventManager - Failed to parse response.response:", parseError);
            console.log("RdnaEventManager - Raw response.response:", response.response.substring(0, 200) + "...");
            return;
          }
        } else {
          // response.response is already an object
          idvOptInCapturedFrameData = response.response as RDNAIDVOptInCapturedFrameConfirmationData;
        }
      } else if (response?.capturedImage) {
        // Direct response object with capturedImage
        idvOptInCapturedFrameData = response as RDNAIDVOptInCapturedFrameConfirmationData;
      } else {
        console.error("RdnaEventManager - Unrecognized response format:", response);
        return;
      }

      console.log("RdnaEventManager - IDV opt-in captured frame confirmation status:", idvOptInCapturedFrameData?.challengeResponse?.status?.statusCode);
      console.log("RdnaEventManager - UserID:", idvOptInCapturedFrameData?.userID);
      console.log("RdnaEventManager - Challenge mode:", idvOptInCapturedFrameData?.challengeMode);
      console.log("RdnaEventManager - Has captured image:", !!idvOptInCapturedFrameData?.capturedImage);
      
      if (idvOptInCapturedFrameData?.capturedImage) {
        console.log("RdnaEventManager - Captured image length:", idvOptInCapturedFrameData.capturedImage.length);
        console.log("RdnaEventManager - Image preview (first 100 chars):", idvOptInCapturedFrameData.capturedImage.substring(0, 100));
      }
      ...
      console.error("RdnaEventManager - Failed to process IDV opt-in captured frame confirmation:", error);
      console.log("RdnaEventManager - Raw response causing error:", response);
    }
  }
  ...
  public cleanup() {
    ...
    // Clear IDV biometric status handlers
    this.idvBiometricOptInStatusHandler = undefined;
    this.idvBiometricOptOutStatusHandler = undefined;
    this.idvOptInCapturedFrameConfirmationHandler = undefined;
