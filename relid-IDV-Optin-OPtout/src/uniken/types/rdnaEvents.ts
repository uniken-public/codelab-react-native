...
/**
 * RDNA IDV Opt-In Captured Frame Confirmation Data
 * IDV opt-in captured frame confirmation event - triggered to confirm captured biometric frame
 */
export interface RDNAIDVOptInCapturedFrameConfirmationData extends RDNAEvent {
  response?: string;
  capturedImage?: string; // Base64 encoded JPEG image data
  userID?: string;
  challengeMode?: number;
}
...
// IDV Biometric Status Callbacks
export type RDNAIDVBiometricOptInStatusCallback = (data: RDNAIDVBiometricOptInStatusData) => void;
export type RDNAIDVBiometricOptOutStatusCallback = (data: RDNAIDVBiometricOptOutStatusData) => void;
export type RDNAIDVOptInCapturedFrameConfirmationCallback = (data: RDNAIDVOptInCapturedFrameConfirmationData) => void;
/**
 * RDNA IDV Biometric Opt-In Status Response Data
 * Response from biometric opt-in confirmation - triggered after user confirms biometric enrollment
 */
export interface RDNAIDVBiometricOptInStatusData extends RDNAEvent {
  userID?: string;
  challengeMode?: number;
  status?:{
    statusCode?: number;
...
/**
 * RDNA IDV Biometric Opt-Out Status Response Data  
 * Response from biometric opt-out confirmation - triggered after user declines biometric enrollment
 */
export interface RDNAIDVBiometricOptOutStatusData extends RDNAEvent {
  userID?: string;
  challengeMode?: number;
}
