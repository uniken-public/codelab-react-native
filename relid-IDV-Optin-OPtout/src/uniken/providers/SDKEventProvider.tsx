...
export const SDKEventProvider: React.FC<SDKEventProviderProps> = ({ children }) => {
  ...
  const handleGetIDVSelfieProcessStartConfirmation = useCallback((data: RDNAGetIDVSelfieProcessStartConfirmationData) => {
    ...
    // Following the same pattern as Selfie scan confirmation navigation
   ...
   * Navigates to confirmation screen when capturedImage is received
   */
  const handleIDVOptInCapturedFrameConfirmation = useCallback((data: RDNAIDVOptInCapturedFrameConfirmationData) => {
    console.log('SDKEventProvider - IDV opt-in captured frame confirmation event received',data);
    ...
    if (data) {
      ...
      console.log('SDKEventProvider - Image data length:', data);
      
      // Navigate to the IDV opt-in captured frame confirmation screen
      ...
      console.log('SDKEventProvider - No captured image found, not navigating');
      
      // Log for debugging but don't navigate
      if (data?.error?.longErrorCode !== 0) {
        console.warn('SDKEventProvider - Frame confirmation error:', data?.error?.errorString);
      }
    }
  ...
  const handleBiometricOptInStatus = useCallback((data: RDNAIDVBiometricOptInStatusData) => {
    console.log('BiometricOptInScreen - Biometric opt-in status received:', data);
    
    if (data?.error.shortErrorCode === 0 && data?.status.statusCode === 100) {
      // Success - navigate to dashboard
      console.log('BiometricOptInScreen - Biometric opt-in successful, navigating to dashboard');
      Alert.alert(
        'Success',                    // Title
    ...
    } else if (data.status.statusCode === 110) {
      // Password expiry - navigate to welcome screen
      console.log('BiometricOptInScreen - Password expiry detected, navigating to welcome');
      // navigation.navigate('TutorialHome' as never);
      NavigationService.navigateOrUpdate('IDVBiometricOptInConsentScreen', {
        title: 'Save Template',
        userDetails: data,
      });
      ...
      // Handle error cases
      console.log('BiometricOptInScreen - Biometric opt-in failed:', data.error.errorString);
     
      Alert.alert(
        'Error',                    // Title
        data?.error?.errorString,       // Message
        [
          {
            text: "OK",
            onPress: () => {
  ...
  const handleBiometricOptOutStatus = useCallback((data: RDNAIDVBiometricOptOutStatusData) => {
    console.log('BiometricOptOutScreen - Biometric opt-out status received:', data);
