...
class RdnaEventManager {
  ...
  private updateCredentialResponseHandler?: RDNAUpdateCredentialResponseCallback;
  ...
  private getNotificationHistoryHandler?: RDNAGetNotificationHistoryCallback;

 // LDA management handlers
  private deviceAuthManagementStatusHandler?: RDNADeviceAuthManagementStatusCallback;

  // Data signing handlers
  private dataSigningResponseHandler?: RDNADataSigningResponseCallback;
  ...
  private registerEventListeners() {
    ...
    this.listeners.push(
      ...
      this.rdnaEmitter.addListener('onUpdateCredentialResponse', this.onUpdateCredentialResponse.bind(this)),
      ...
      this.rdnaEmitter.addListener('onGetNotificationsHistory', this.onGetNotificationHistory.bind(this)),

      // LDA management event listeners
      this.rdnaEmitter.addListener('onDeviceAuthManagementStatus', this.onDeviceAuthManagementStatus.bind(this)),

      // Data signing event listeners
      this.rdnaEmitter.addListener('onAuthenticateUserAndSignData', this.onAuthenticateUserAndSignData.bind(this)),
   ...
   * @param response Raw response from native SDK containing update credential response data
   */
  private onUpdateCredentialResponse(response: RDNAJsonResponse) {
    console.log("RdnaEventManager - Update credential response event received");

    try {
      const updateCredentialData: RDNAUpdateCredentialResponseData = JSON.parse(response.response);
      console.log("RdnaEventManager - Update credential response for user:", updateCredentialData.userID);
      console.log("RdnaEventManager - Credential type:", updateCredentialData.credType);
      console.log("RdnaEventManager - Status code:", updateCredentialData.status.statusCode);
      console.log("RdnaEventManager - Status message:", updateCredentialData.status.statusMessage);
  ...
  private onGetNotificationHistory(response: RDNAJsonResponse) {
    console.log("RdnaEventManager - Get notification history event received");

    try {
      const historyData: RDNAGetNotificationHistoryData = JSON.parse(response.response);

      console.log("RdnaEventManager - Get notification history data:", {
        errCode: historyData.errCode,
        statusCode: historyData.pArgs?.response?.StatusCode,
        statusMsg: historyData.pArgs?.response?.StatusMsg,
        totalCount: historyData.pArgs?.response?.ResponseData?.total_count || 0,
        historyCount: historyData.pArgs?.response?.ResponseData?.history?.length || 0
      });
      ...
      console.error("RdnaEventManager - Failed to parse get notification history response:", error);
    }
  }
  ...
  private onDeviceAuthManagementStatus(response: RDNAJsonResponse) {
    ...
    try {
      const authManagementStatusData: RDNADeviceAuthManagementStatusData = JSON.parse(response.response);
   ...
   * @param response Raw response from native SDK containing signature data
   */
  private onAuthenticateUserAndSignData(response: RDNAJsonResponse) {
    console.log("RdnaEventManager - Data signing response event received");

    try {
      const signingResponseData: RDNADataSigningResponse = JSON.parse(response.response);
      console.log("RdnaEventManager - Data signing completed:", {
        authLevel: signingResponseData.authLevel,
        authenticationType: signingResponseData.authenticationType,
        payloadLength: signingResponseData.dataPayloadLength,
        signatureIdLength: signingResponseData.dataSignatureID?.length || 0,
        statusCode: signingResponseData.status.statusCode,
        errorCode: signingResponseData.error.shortErrorCode
      });

      if (this.dataSigningResponseHandler) {
        this.dataSigningResponseHandler(signingResponseData);
      }
    } catch (error) {
  ...
  private onGetRegistredDeviceDetails(response: RDNAJsonResponse) {
    ...
    try {
      const deviceDetailsData: RDNAGetRegisteredDeviceDetailsData = JSON.parse(response.response);

      console.log("RdnaEventManager - Get registered device details data:", {
      ...
      console.error("RdnaEventManager - Failed to parse get registered device details response:", error);
    }
  }
  ...
  private onUpdateDeviceDetails(response: RDNAJsonResponse) {
    ...
    try {
      const updateDeviceData: RDNAUpdateDeviceDetailsData = JSON.parse(response.response);

      console.log("RdnaEventManager - Update device details data:", {
      ...
      console.error("RdnaEventManager - Failed to parse update device details response:", error);
    }
  }
  ...
  public setUpdateCredentialResponseHandler(callback?: RDNAUpdateCredentialResponseCallback): void {
  ...
  // Getter methods for preserving existing handlers
  ...
  public setDeviceAuthManagementStatusHandler(callback?: RDNADeviceAuthManagementStatusCallback): void {
    this.deviceAuthManagementStatusHandler = callback;
  }

  // Data Signing Handler Setters

  public setDataSigningResponseHandler(callback?: RDNADataSigningResponseCallback): void {
  ...
  public cleanup() {
    ...
    this.getNotificationHistoryHandler = undefined;

    // Clear LDA management handlers
    this.deviceAuthManagementStatusHandler = undefined;

    // Clear data signing handlers
    this.dataSigningResponseHandler = undefined;
