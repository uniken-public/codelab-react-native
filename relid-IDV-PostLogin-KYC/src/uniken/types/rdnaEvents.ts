 ...
 * Event indicating credentials can be updated
 */
...
// --- IDV Document Scan Events ---

/**
 * RDNA Get IDV Document Scan Process Start Confirmation Data
 * IDV document scan process start request event
 */
export interface RDNAGetIDVDocumentScanProcessStartConfirmationData extends RDNAEvent {
  userID: string;
  idvWorkflow: number;
  documentType?: string;
  documentScanDetails?: {
 ...
 * IDV selfie capture process start request event
 */
export interface RDNAGetIDVSelfieProcessStartConfirmationData extends RDNAEvent {
  ...
  selfieDetails?: {
    ...
    multipleAnglesRequired: boolean;
...
/**
 * RDNA Get IDV Selfie Confirmation Data
 * IDV selfie confirmation request event - triggered to confirm captured selfie
 */
export interface RDNAGetIDVSelfieConfirmationData extends RDNAEvent {
  userID: string;
  challengeMode: number;
  response_data: {
    action_buttons: {
...
/**
 * RDNA Get IDV Biometric Opt-In Consent Data
 * IDV biometric opt-in consent request event - triggered to get user consent for biometric storage
 */
export interface RDNAGetIDVBiometricOptInConsentData extends RDNAEvent {
  userID: string;
  challengeMode: number;
  biometricTemplate?: {
    templateData?: string;
...
/**
 * RDNA Get IDV Confirm Document Details Data
 * IDV document confirmation event with scan results
 */
export interface RDNAGetIDVConfirmDocumentDetailsData extends RDNAEvent {
  challengeMode: number;
  response_data: {
    version?: string;
...
/**
 * RDNA IDV Activated Customer KYC Response Data
 * Response event triggered after initiateActivatedCustomerKYC API call
 * Contains KYC workflow details and next steps for customer identity verification
 * Following current project's RDNAEvent structure for consistency
 */
...
// IDV Document Scan Callbacks
export type RDNAGetIDVDocumentScanProcessStartConfirmationCallback = (data: RDNAGetIDVDocumentScanProcessStartConfirmationData) => void;
export type RDNAGetIDVSelfieProcessStartConfirmationCallback = (data: RDNAGetIDVSelfieProcessStartConfirmationData) => void;
export type RDNAGetIDVConfirmDocumentDetailsCallback = (data: RDNAGetIDVConfirmDocumentDetailsData) => void;
export type RDNAGetIDVSelfieConfirmationCallback = (data: RDNAGetIDVSelfieConfirmationData) => void;
export type RDNAGetIDVBiometricOptInConsentCallback = (data: RDNAGetIDVBiometricOptInConsentData) => void;
