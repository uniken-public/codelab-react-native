...
import type {
  ...
  RDNAGetIDVDocumentScanProcessStartConfirmationCallback,
  ...
  RDNAGetIDVConfirmDocumentDetailsCallback,
  RDNAGetIDVSelfieConfirmationCallback,
  RDNAGetIDVBiometricOptInConsentData,
  RDNAGetIDVBiometricOptInConsentCallback,
  RDNAIDVActivatedCustomerKYCResponseData,
  RDNAIDVActivatedCustomerKYCResponseCallback
...
class RdnaEventManager {
  ...
  // IDV document scan handlers
  private getIDVDocumentScanProcessStartConfirmationHandler?: RDNAGetIDVDocumentScanProcessStartConfirmationCallback;
  private getIDVSelfieProcessStartConfirmationHandler?: RDNAGetIDVSelfieProcessStartConfirmationCallback;
  private getIDVConfirmDocumentDetailsHandler?: RDNAGetIDVConfirmDocumentDetailsCallback;
  private getIDVSelfieConfirmationHandler?: RDNAGetIDVSelfieConfirmationCallback;
  private getIDVBiometricOptInConsentHandler?: RDNAGetIDVBiometricOptInConsentCallback;
  
  // IDV KYC handlers
  private idvActivatedCustomerKYCResponseHandler?: RDNAIDVActivatedCustomerKYCResponseCallback;
  ...
  /**
   * Ensures callback execution on the main thread to prevent UI API calls on background threads
   * @param callback Function to execute on main thread
   */
  private dispatchToMainThread(callback: () => void) {
    // Use setTimeout with 0ms to dispatch to the next tick on the main thread
    // This is the React Native way to ensure main thread execution
    setTimeout(() => {
      callback();
    }, 0);
  }
  ...
  private registerEventListeners() {
    ...
    this.listeners.push(
      ...
      // IDV document scan event listeners
      this.rdnaEmitter.addListener('getIDVDocumentScanProcessStartConfirmation', this.onGetIDVDocumentScanProcessStartConfirmation.bind(this)),
      this.rdnaEmitter.addListener('getIDVSelfieProcessStartConfirmation', this.onGetIDVSelfieProcessStartConfirmation.bind(this)),
      this.rdnaEmitter.addListener('getIDVConfirmDocumentDetails', this.onGetIDVConfirmDocumentDetails.bind(this)),
      this.rdnaEmitter.addListener('getIDVSelfieConfirmation', this.onGetIDVSelfieConfirmation.bind(this)),
      this.rdnaEmitter.addListener('getIDVBiometricOptInConsent', this.onGetIDVBiometricOptInConsent.bind(this)),
      
      // IDV KYC event listeners
      this.rdnaEmitter.addListener('onIDVActivatedCustomerKYCResponse', this.onIDVActivatedCustomerKYCResponse.bind(this)),
  ...
  /**
   * Handles IDV document scan process start confirmation request events
   * @param response Raw response from native SDK containing IDV document scan data
   */
  private onGetIDVDocumentScanProcessStartConfirmation(response: RDNAJsonResponse) {
    console.log("RdnaEventManager - Get IDV document scan process start confirmation event received");

    try {
      const idvDocumentScanData: RDNAGetIDVDocumentScanProcessStartConfirmationData = JSON.parse(response.response);
      console.log("RdnaEventManager - Get IDV document scan confirmation status:", idvDocumentScanData);
      console.log("RdnaEventManager - UserID:", idvDocumentScanData.userID, "IDV Workflow:", idvDocumentScanData.idvWorkflow);

      if (this.getIDVDocumentScanProcessStartConfirmationHandler) {
        this.dispatchToMainThread(() => {
          this.getIDVDocumentScanProcessStartConfirmationHandler?.(idvDocumentScanData);
        });
      }
    } catch (error) {
      console.error("RdnaEventManager - Failed to parse get IDV document scan process start confirmation:", error);
    }
  }

  /**
   * Handles IDV selfie process start confirmation request events
   * @param response Raw response from native SDK containing IDV selfie process start data
   */
  ...
  /**
   * Handles IDV confirm document details request events
   * @param response Raw response from native SDK containing document confirmation data
   */
  private onGetIDVConfirmDocumentDetails(response: RDNAJsonResponse) {
    console.log("RdnaEventManager - Get IDV confirm document details event received");

    try {
      const idvConfirmData: RDNAGetIDVConfirmDocumentDetailsData = JSON.parse(response.response);
      console.log("RdnaEventManager - IDV document confirmation status:", idvConfirmData);
      console.log("RdnaEventManager - Challenge mode:", idvConfirmData?.challengeMode);
      console.log("RdnaEventManager - Document version:", idvConfirmData?.response_data?.version);

      if (this.getIDVConfirmDocumentDetailsHandler) {
        this.dispatchToMainThread(() => {
          this.getIDVConfirmDocumentDetailsHandler?.(idvConfirmData);
        });
      }
    } catch (error) {
      console.error("RdnaEventManager - Failed to parse get IDV confirm document details:", error);
    }
  }

  /**
   * Handles IDV selfie confirmation events
   * @param response Raw response from native SDK containing selfie confirmation details
   */
  private onGetIDVSelfieConfirmation(response: RDNAJsonResponse) {
    console.log("RdnaEventManager - Get IDV selfie confirmation event received");

    try {
      const idvSelfieConfirmData: RDNAGetIDVSelfieConfirmationData = JSON.parse(response.response);
      console.log("RdnaEventManager - IDV selfie confirmation status:", idvSelfieConfirmData?.challengeResponse?.status?.statusCode);
      console.log("RdnaEventManager - Challenge mode:", idvSelfieConfirmData?.challengeMode);
      console.log("RdnaEventManager - UserID:", idvSelfieConfirmData?.userID);

      if (this.getIDVSelfieConfirmationHandler) {
        this.dispatchToMainThread(() => {
          this.getIDVSelfieConfirmationHandler?.(idvSelfieConfirmData);
        });
      }
    } catch (error) {
      console.error("RdnaEventManager - Failed to parse get IDV selfie confirmation:", error);
    }
  }

  /**
   * Handles IDV biometric opt-in consent events
   * @param response Raw response from native SDK containing biometric opt-in consent details
   */
  ...
  /**
   * Handles IDV activated customer KYC response events
   * @param response Raw response from native SDK containing KYC response data
   */
  private onIDVActivatedCustomerKYCResponse(response: RDNAJsonResponse) {
    console.log("RdnaEventManager - IDV activated customer KYC response event received");

    try {
      const kycResponseData: RDNAIDVActivatedCustomerKYCResponseData = JSON.parse(response.response);
      console.log("RdnaEventManager - IDV KYC response status:", kycResponseData);
      console.log("RdnaEventManager - UserID:", kycResponseData.userID);
      console.log("RdnaEventManager - KYC Workflow:", kycResponseData.kycWorkflow?.workflowType);
      console.log("RdnaEventManager - Reason:", kycResponseData.reason);

      if (this.idvActivatedCustomerKYCResponseHandler) {
        this.dispatchToMainThread(() => {
          this.idvActivatedCustomerKYCResponseHandler?.(kycResponseData);
        });
      }
    } catch (error) {
      console.error("RdnaEventManager - Failed to parse IDV activated customer KYC response:", error);
    }
  }
  ...
  // IDV Document Scan Handler Setters

  public setGetIDVDocumentScanProcessStartConfirmationHandler(callback?: RDNAGetIDVDocumentScanProcessStartConfirmationCallback): void {
    this.getIDVDocumentScanProcessStartConfirmationHandler = callback;
  }
  ...
  public setGetIDVConfirmDocumentDetailsHandler(callback?: RDNAGetIDVConfirmDocumentDetailsCallback): void {
    this.getIDVConfirmDocumentDetailsHandler = callback;
  }

  public setGetIDVSelfieConfirmationHandler(callback?: RDNAGetIDVSelfieConfirmationCallback): void {
    this.getIDVSelfieConfirmationHandler = callback;
  }
  ...
  // IDV KYC Handler Setters

  public setIDVActivatedCustomerKYCResponseHandler(callback?: RDNAIDVActivatedCustomerKYCResponseCallback): void {
    this.idvActivatedCustomerKYCResponseHandler = callback;
  }
  ...
  public cleanup() {
    ...
    // Clear IDV document scan handlers
    this.getIDVDocumentScanProcessStartConfirmationHandler = undefined;
    this.getIDVSelfieProcessStartConfirmationHandler = undefined;
    this.getIDVConfirmDocumentDetailsHandler = undefined;
    this.getIDVSelfieConfirmationHandler = undefined;
    this.getIDVBiometricOptInConsentHandler = undefined;
    
    // Clear IDV KYC handlers
    this.idvActivatedCustomerKYCResponseHandler = undefined;
