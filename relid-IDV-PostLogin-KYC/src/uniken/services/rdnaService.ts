...
export class RdnaService {
  ...
  /**
   * Initiates activated customer KYC process for post-login KYC workflow
   *
   * This method initiates the KYC (Know Your Customer) process for already activated customers
   * who need to complete additional identity verification after login. It's typically used
   * for post-login KYC scenarios where customers need to verify their identity documents.
   * After successful API call, the SDK will trigger onIDVActivatedCustomerKYCResponse event.
   * Uses sync response pattern similar to other API methods.
   *
   * @see https://developer.uniken.com/docs/initiateactivatedcustomerkyc
   *
   * Response Validation Logic (following reference app pattern):
   * 1. Check error.longErrorCode: 0 = success, > 0 = error
   * 2. An onIDVActivatedCustomerKYCResponse event will be triggered with KYC workflow details
   * 3. Async events will be handled by event listeners for subsequent IDV steps
   *
   * @param reason The reason for initiating the KYC process (e.g., "Post Login KYC", "Account Verification")
   * @returns Promise<RDNASyncResponse> that resolves with sync response structure
   */
